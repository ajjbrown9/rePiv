...
def extract_pivot(
    ...
    # Keep only columns that actually have a header label.
    ...
    header_map = mapping_from_records(records)
    col_names = [header_map[n] for n in raw_names]
...
def ingest(
    ...
    for path in paths:
        ...
        try:
            ...
            df.insert(1, "source_sha256", digest)
            df.insert(2, "ingested_at", stamp)
...
def extract_hierarchical_pivot(
    ...
    total_re = total_re or TOTAL_RE
    non_total = body[~body.apply(
        lambda r: any(isinstance(v, str) and total_re.search(v) for v in r.values),
        axis=1)]
    value_cols, dim_cols = [], []
    for c in col_names:
        (value_cols if _looks_numeric(non_total[c]) >= value_threshold
         else dim_cols).append(c)
    if not value_cols:
        raise ValueError("No numeric value columns detected to unpivot.")
    if not dim_cols:
...
# mode='auto' sniffs; force with mode='flat' or mode='hierarchical'.
# Applies the DECLARED business hierarchy: puts the canonical level columns in
# business order and checks each file actually exposes those exact levels after
# aliasing -- appending HIGH rows to the report for any missing or unexpected
# level, so cross-file misalignment surfaces instead of silently passing.
# Applies the DECLARED business hierarchy: every declared level MUST be present
# as a column (schema is enforced either way). A level that is genuinely absent
...
def read_total_row(path, sheet=0, aliases=None, fuzzy=True,
    ...
    # against the wrong number, which is worse than having no gate at all.
    total_re = total_re or TOTAL_RE
    probe = body[~body.apply(
        lambda r: any(isinstance(v, str) and total_re.search(v) for v in r.values),
        axis=1)]
